...
class OrderController {
    ...
    addItem(req, res) {
        ...
        connection.query(query, [req.userjwt.user_id], (error, results) => {
            if (error) {
            ...
            if (results.length === 0) {
                ...
                connection.query(query_insert, [req.userjwt.user_id], (error, results) => {
                    if (error) {
                        console.error('Lỗi truy vấn cơ sở dữ liệu:', error);
                        return res.status(500).json({ message: 'Lỗi máy chủ' });
                    }
    ...
    PostConfirm(req, res) {
        ...
        const user_id = req.userjwt.user_id;
        const query = 'SELECT * FROM `orders` WHERE user_id = ? and status IS NULL';
    ...
    dashboardStatus(req, res) {
        ...
        Promise.all([getTotalSales(), getTotalOrder(), getTotalReturn()])
            .then(() => {
                res.status(200).json({
    ...
    getDetailOrderByUser(req, res) {
        ...
        const getListBook = () => {
            return new Promise((resolve, reject) => {
                ...
                connection.query(query, [req.userjwt.user_id, order_id], (error, results) => {
                    if (error) {
                        console.error('Lỗi truy vấn cơ sở dữ liệu:', error);
                        reject(error);
                    }
        ...
        const getTotalOrder = () => {
            return new Promise((resolve, reject) => {
                ...
                connection.query(query2, [order_id, req.userjwt.user_id], (error, results) => {
                    if (error) {
                        console.error('Lỗi truy vấn cơ sở dữ liệu:', error);
                        reject(error);
                    }
