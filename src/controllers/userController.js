...
class UserController {
    ...
    login(req, res) {
        ...
        connection.query(query, [username], (error, results) => {
            ...
            bcrypt.compare(password, user.password, (err, isMatch) => {
                ...
                // req.userjwt.username = user.username;
                // req.userjwt.user_id = user.id;
                ...
                // Tạo JWT và ký nó bằng secret key
                ...
                // res.cookie('sessionId', req.userjwt);
                return res.status(200).json({ message: 'Đăng nhập thành công', isLogin: true, user, token });
            });
        });
    ...
    adminLogin(req, res) {
        ...
        connection.query(query, [username], (error, results) => {
            ...
            bcrypt.compare(password, user.password, (err, isMatch) => {
                ...
                delete user.password; // Xóa mật khẩu khỏi đối tượng người dùng trước khi trả về
                // res.cookie('sessionId', req.userjwt);
                return res.status(200).json({ message: 'Đăng nhập thành công', isAdmin: true, user, token });
            });
        });
    }
    ...
    islogin(req, res) {
        ...
        // console.log('is login: ', req.userjwt);
        // console.log(req.userjwt);
        if (req.userjwt.username) {
            ...
            connection.query(query, [req.userjwt.username], (error, results) => {
                if (error) {
                    console.error('Lỗi truy vấn cơ sở dữ liệu:', error);
                    return res.status(500).json({ message: 'Lỗi máy chủ' });
                }

                console.log(req.userjwt.username);
    ...
    getInfo(req, res) {
        ...
        const query = 'SELECT * FROM `users` WHERE username = ?'
        console.log('getinfo: ', req.userjwt);

        connection.query(query, [req.userjwt.username], (err, results) => {
            if (err) throw err;
            ...
            bcrypt.compare(password_old, user.password, (err, isMatch) => {
                ...
                bcrypt.hash(password_new, 10, async (error, hashedPassword) => {
                    if (error) {
                        ...
                        connection.query(updateQuery, [hashedPassword, req.userjwt.username], (error, results) => {
                            if (error) {
                                console.error('Lỗi truy vấn cơ sở dữ liệu:', error);
                                return res.status(500).json({ message: 'Lỗi máy chủ' });
                            }
